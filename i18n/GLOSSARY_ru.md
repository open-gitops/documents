...
- ## Автоматическая вытягиваемoсть (Pull)
    ...
    **Принцип 3** ([Автоматически вытягиваемый](./PRINCIPLES_ru.md)) указывает, что желаемое состояние должно быть «вытянуто» (*pull*), а не передано из вне (*push*), главным образом потому, что программные агенты должны иметь возможность в любой момент времени получать доступ к [желаемому состоянию](#желаемое-состояние) из [хранилища состояния](#хранилище-состояния), а не только когда происходит намеренное изменение в хранилище состояния, инициирующее событие отправления/передачи (*push*). Это является необходимым условием для того, чтобы [согласование](#согласование) происходило [непрерывно](#непрерывность), как указано в **принципе 4** (Непрерывное согласование). Обратите внимание, что – в отличие от традиционного CI/CD (непрерывная интеграция и непрерывное развертывание), где автоматизация обычно запускается заранее заданными триггерами – в GitOps согласование запускается в любое время, когда происходит расхождение. Расхождение может быть связано с тем, что фактическое состояние непреднамеренно [отклоняется](#отклонение-drift) от деклараций желаемого состояния, а не только из-за того, что была намеренно изменена версия декларации нового желаемого состояния.
